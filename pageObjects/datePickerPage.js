...
export class DatePickerPage {
    ...
    async selectDate({
        ...
        if (datePicker === "date") {
            ...
            await this.setDate({ day: day, month: month, year: year })

            return await expect(await this.selectDateInput).toHaveAttribute("value", dateString);

        } else if (datePicker === "dateAndTime") {
            let timeConverted = this.convertTime(time);
            dateString = `${month} ${day}, ${year} ${timeConverted}`
            await this.dateAndTimeInput.click();
            await expect(await this.datePickerWindow).toBeAttached();
            await this.setDateAndTime({ day: day, month: month, year: year, time: time })

            return await expect(await this.dateAndTimeInput).toHaveAttribute("value", dateString);
        } else
    ...
    async setDateAndTime({ month = "", year = "", time = "", day = "" }) {
